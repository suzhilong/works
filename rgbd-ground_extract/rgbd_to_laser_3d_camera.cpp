...
void rotate_to_localGroundFrame(pcl::PointCloud<PointT>::Ptr cloud , pcl::ModelCoefficients::ConstPtr coef , ros::Time time);
...
int main (int argc, char** argv)
{
  ...
  scan_msg.header.frame_id = "/camera_depth_frame";//"local_ground_frame";
...
void ground_boundary_extract(const sensor_msgs::PointCloud2ConstPtr& input)
{
  ...
  pass.setFilterLimits (0.60, 0.65);//camera hight: cruzr is 0.55-0.6; turtlebot is 0.25-0.35
...
{//get outline of ground
  ...
  for (int i=0; i<cloud->size(); i++){
    ...
    if ( a == 1)
    {
      if (cloud->points[i].z>1.6)//change 1 to SAFE_DISTANCE
      {//把最近的边去掉
...
//  std::cout << "project cloud size is " << cloud->size() << std::endl;
  // std::cout << "---start--------rotate_to_localGroundFrame----------" << std::endl;
  ...
  br.sendTransform(tf::StampedTransform(transform , time , "camera_depth_frame" , "local_ground_frame"));
  ...
  */
  // std::cout << "---end--------rotate_to_localGroundFrame----------" << std::endl;
}
...
void segmentation_with_cluster_cb (const sensor_msgs::PointCloud2ConstPtr& input)
{
  ...
  {//have no plane
    ...
    {//MAX_COUNT_NO_GROUND=20, hasFirstGroundAxis init is false,
      ...
      // rotate_to_localGroundFrame(rawCloud.makeShared() ,boost::make_shared<pcl::ModelCoefficients>(coefficientsOfPlane) , t_base);
      // rotate_to_localGroundFrame(cloudForPlaneSeg.makeShared() ,boost::make_shared<pcl::ModelCoefficients>(coefficientsOfPlane) , t_base);
