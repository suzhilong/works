...
int main (int argc, char** argv)
{
  ...
  scan_msg.header.frame_id = "world";//"/camera_depth_frame";
  ...
  ground_pub2 = nh.advertise<sensor_msgs::PointCloud2>("/pointcloud_ground2", 10);//ground
  ...
  ros::Subscriber downsamplingPcSub = nh.subscribe("/cloud_for_plane",1, ground_boundary_extract);
...
void ground_boundary_extract(const sensor_msgs::PointCloud2ConstPtr& input)
{
  ...
  pass.setFilterLimits (0.3, 0.35);//camera hight: cruzr is 0.55-0.6; turtlebot is 0.25-0.35
...
{//get outline of ground
  ...
  // std::cout<<"normal size is "<< normals->size()<<endl;
  ...
  boundPoints->header.frame_id = input_frame_id;
  ...
  for (int i=0; i<cloud->size(); i++){
    ...
    if ( a == 1)
    {
      if (cloud->points[i].z>0.5)//change 1 to SAFE_DISTANCE
      {//把最近的边去掉
...
//  // std::cout << "project cloud size is " << cloud->size() << std::endl;
  // // std::cout << "---start--------rotate_to_worldFrame----------" << std::endl;
  ...
  br.sendTransform(tf::StampedTransform(transform , time , "camera_depth_frame" , "world"));
  ...
  cloudInGround->header.frame_id = "world";
  ...
  */
  // // std::cout << "---end--------rotate_to_worldFrame----------" << std::endl;
}
...
void segmentation_with_cluster_cb (const sensor_msgs::PointCloud2ConstPtr& input)
{
  ...
  {//filt cloudForPlaneSeg (-0.1~0.2), first will not do, only do when hasFirstGroundAxis
    ...
    pass.setFilterLimits (-1, 1.5);//-0.1~0.2//-1~5 //high of robot
    pass.filter (*indicesPT);
  ...
  {//have no plane
    ...
    {//MAX_COUNT_NO_GROUND=20, hasFirstGroundAxis init is false,
      ...
      // rotate_to_worldFrame(rawCloud.makeShared() ,boost::make_shared<pcl::ModelCoefficients>(coefficientsOfPlane) , t_base);
      // rotate_to_worldFrame(cloudForPlaneSeg.makeShared() ,boost::make_shared<pcl::ModelCoefficients>(coefficientsOfPlane) , t_base);
